...
@section('titulo','Registrar Asignaturas')
...
<div class="container">     
    ...
    <div class="fw-bold border-bottom mb-4 animate-slide-in">
        <h2>Listado de asignaturas registradas.</h2>
    </div>
    ...
    <nav class="navbar float-right">         
        <form class="form-inline my-2 my-lg-0" method="GET">         
            <input name="buscarpor" class="form-control mr-sm2" type="search" placeholder="Buscar por Código/Nombre" arialabel="Search" value="{{ $buscarpor }}">             
    ...
    <div class="table-responsive">
        <table id="add-row" class="display table table-striped table-hover" >
            ...
            <tbody>
                @foreach($asignaturas as $itemAsignatura)                                            
                    <tr>                                         
                        <td class="text-center">{{ $itemAsignatura->asignatura_id }}</td>
                        <td class="text-center">{{ $itemAsignatura->codigo }}</td>
                        <td>{{ $itemAsignatura->nombre }}</td>
                        <td>{{ $itemAsignatura->descripcion }}</td>
                        <td class="text-center">{{ $itemAsignatura->horas_semanales }}</td>                                         
                    </tr>                  
                @endforeach         
